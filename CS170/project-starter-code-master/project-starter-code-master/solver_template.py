...
def solve(list_of_kingdom_names, starting_kingdom, adjacency_matrix, params=[]):
	...
	terminal_nodes = [x for x in range(len(list_of_kingdom_names))]
	...
	cutout = []
	t = 0
	while t < len(adjacency_matrix):
		numnums = 0
		for element in adjacency_matrix[t]:
	...
	while let1 < len(adjacency_matrix):
		...
		let1+=1
	#run DFS
	traversal, lastunique= DFS(subgraph, realindexes.index(startingnode))
	last_kingdom_real = realindexes[traversal[len(traversal)-1]]
	starting_kingdom_real = startingnode
	conquers = list(set(traversal))
	...
	for b in realtraversal:
		namedtraversalfirst.append(list_of_kingdom_names[b])
	for c in conquers:
		nameddominatefirst.append(list_of_kingdom_names[c])
	return namedtraversalfirst, nameddominatefirst
	"""
	startingnode = list_of_kingdom_names.index(starting_kingdom)
