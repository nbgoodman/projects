...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        0.0
        """
        randnum = random.random() * self.total()
        sum = 0
        for key, value in self.items():
            if randnum <= sum + value:
                return key
            sum += value
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        curloc = gameState.getPacmanPosition()
        jail = self.getJailPosition()
        for pos in self.allPositions:
            self.beliefs[pos] *= self.getObservationProb(observation, curloc, pos, jail)
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for item in self.particles:
            newdist[item] += self.getObservationProb(observation, pacmanpos, item, jail)
        newlist = []
        if newdist.total() == 0:
            self.initializeUniformly(gameState)
        else:
    ...
    def elapseTime(self, gameState):
        ...
        for pos, prob in self.getBeliefDistribution().items():
            newPosDist = self.getPositionDistribution(gameState, pos)
            for num in range(int(prob*self.numParticles)):
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        uniform prior.
        """
        stuff = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        newlist = []
        i = 0
        while i < self.numParticles:
            newlist.append(stuff[i % len(stuff)])
            i += 1
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for part in self.particles:
            ...
            for i in range(self.numGhosts):
                prob *= self.getObservationProb(observation[i], pacmanpos, part[i], self.getJailPosition(i))
            newdist[part] += prob
        if newdist.total() == 0:
            self.initializeUniformly(gameState)
        else:
