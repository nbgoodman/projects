...
unsigned write_pass_one(FILE* output, const char* name, char** args, int num_args) {
    ...
    if (strcmp(name, "li") == 0) {
      ...
      int check = translate_num(&imm, args[1], -2147483648, 2147483647);
      if (check == 0) {
        int immupper = ((imm & 0xffff0000) >> 16);
        int immlower = (imm & 0x0000ffff);
        fprintf(output, "%s %s %i\n", "lui", "$at", immupper);
        fprintf(output, "%s %s %s %i\n", "ori", args[0], "$at", immlower);
        return 2;
      }
    return 0;
    ...
    return 0;
        ...
        int sp_modify = 0;
        int arg0 = translate_reg(args[0]);
        int arg1 = translate_reg(args[1]);
        int arg2 = translate_reg(args[2]);
        ...
        fprintf(output, "%s %s %s %i \n", "addiu", "$sp", "$sp", -sp_modify);
        int reverse_save = 0;
        if (arg0 != 0) {
          fprintf(output, "%s %s %i %s \n", "sw", args[0], reverse_save, "$sp");
          reverse_save += 4;
        }
        if (arg1 != 0) {
          fprintf(output, "%s %s %i %s \n", "sw", args[1], reverse_save, "$sp");
          reverse_save += 4;
        }
        if (arg2 != 0) {
          fprintf(output, "%s %s %i %s \n", "sw", args[2], reverse_save, "$sp");
          reverse_save += 4;
        }
        return (reverse_save / 4) + 1;
      ...
      int reverse_save = 0;
      if (arg0 != 0) {
        fprintf(output, "%s %s %i %s \n", "lw", args[0], reverse_save, "$sp");
        reverse_save += 4;
      }
      if (arg1 != 0) {
        fprintf(output, "%s %s %i %s \n", "lw", args[1], reverse_save, "$sp");
        reverse_save += 4;
      }
      if (arg2 != 0) {
        fprintf(output, "%s %s %i %s \n", "lw", args[2], reverse_save, "$sp");
        reverse_save += 4;
      }
      fprintf(output, "%s %s %s %i \n", "addiu", "$sp", "$sp", reverse_save);
      return ((reverse_save) / 4) + 1;
    }
...
int write_lui(uint8_t opcode, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = (opcode << 26) + (rs << 16) + imm;
    write_inst_hex(output, instruction);
    return 0;
}
...
int write_branch(uint8_t opcode, FILE* output, char** args, size_t num_args, uint32_t addr, SymbolTable* symtbl) {
    ...
    int branch_check = can_branch_to(addr, label_addr);
    if (label_addr == -1 || rs == -1 || rt == -1 || branch_check == 0) {
      return -1;
    }
    ...
    //}
    //else {
    instruction = (opcode << 26) + (rs << 21) + (rt << 16) + offset;
    //}
    write_inst_hex(output, instruction);
    return 0;
}
