...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    while(fgets(buf, BUF_SIZE, input)) {
      ...
      char* label_check = token;
      int label = add_if_label(input_line, label_check, byte_offset, symtbl);
      if (label == 1) {
      ...
      if (label == -1) {
        ...
        int num_args = 0;
        char* name = token;
        int parsing = parse_args(input_line, args, &num_args);
        if (parsing == -1) {
          ret_code = -1;
        }
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    while (fgets(buf, BUF_SIZE, input)) {
        ...
        int check = translate_inst(output, name, args, num_args, byte_offset, symtbl, reltbl);
        if (check == -1) {
          raise_inst_error(input_line, name, args, num_args);
          ret_code = -1;
        }
