...
class Graph(object):
    ...
    def get_inputs(self, node):
        ...
        """
        final = []
        for thing in self.parentdict[node]:
            final.append(self.get_output(thing))
        return final
    ...
    def add(self, node):
        ...
        self.parentdict[node] = node.get_parents()
        nodeinputs = self.get_inputs(node)
        self.outputdict[node] = node.forward(nodeinputs)
        self.gradientdict[node] = np.zeros_like(self.outputdict[node])
    ...
    def backprop(self):
        ...
        self.gradientdict[loss_node] = np.ones(self.outputdict[loss_node].shape)
        assert np.asarray(self.get_output(loss_node)).ndim == 0
        reversednodes = self.get_nodes()[::-1]
        for elem in reversednodes:
            inputs = self.get_inputs(elem)
            gradient = self.get_gradient(elem)
            vals = elem.backward(inputs, gradient)
            i = 0
            while i < len(inputs):
                self.gradientdict[elem.get_parents()[i]] += vals[i]
                i+=1
        "*** YOUR CODE HERE ***"
    ...
    def step(self, step_size):
        ...
        for node in self.variables:
            node.data = node.data - (self.get_gradient(node) * step_size)
...
class MatrixMultiply(FunctionNode):
    ...
    @staticmethod
    def backward(inputs, gradient):
        a_back = np.matmul(gradient, np.transpose(inputs[1]))
        b_back = np.matmul(np.transpose(inputs[0]), gradient)
...
class ReLU(FunctionNode):
    ...
    def forward(inputs):
        x = inputs[0]
        m = np.maximum(x, np.zeros_like(x))
        return m

    @staticmethod
    def backward(inputs, gradient):
        x = inputs[0]
        l = np.where(x > 0, 1, 0) * gradient
        return [np.array(l)]
...
class SquareLoss(FunctionNode):
    ...
    @staticmethod
    def forward(inputs):
        c = ((inputs[0] - inputs[1]) ** 2) * 0.5
        return np.mean(c)


    @staticmethod
    def backward(inputs, gradient):
        sub1 = (((inputs[0] - inputs[1])) * gradient) / inputs[0].size
        sub2 = (((inputs[1] - inputs[0])) * gradient) / inputs[0].size


        return [np.array(sub1), np.array(sub2)]
