...
def fillObsCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    poslist = [topLeftPos, topRightPos, bottomLeftPos, bottomRightPos]
    for i in range(0, len(poslist)):
        pos = poslist[i]
        houseval = HOUSE_VALS[i]
        for obs_pos in gameState.getHouseWalls(pos):
            obsvar = OBS_VAR_TEMPLATE % obs_pos
            ...
            for assignment in obsfactor.getAllPossibleAssignmentDicts():
                food = assignment[FOOD_HOUSE_VAR] == houseval
                ghost = assignment[GHOST_HOUSE_VAR] == houseval
                if food:
                    ...
                    if assignment[obsvar] == RED_OBS_VAL or assignment[obsvar] == BLUE_OBS_VAL:
                        obsfactor.setProbability(assignment, 0)
                    else:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    joinedfactor = inference.inferenceByVariableElimination(bayesNet, [FOOD_HOUSE_VAR], evidence, eliminationOrder)
    maxprob = float("-inf")
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        probfoodright = 0
        "*** YOUR CODE HERE ***"
        joinedfactor = inference.inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], evidence, eliminationOrder)
        for assignment in joinedfactor.getAllPossibleAssignmentDicts():
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                probfoodleft += joinedfactor.getProbability(assignment)
            elif assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL and assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL:
                probfoodright += joinedfactor.getProbability(assignment)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, newevidence in self.getExplorationProbsAndOutcomes(evidence):
            something = evidence
            something.update(newevidence)
            leftenter, rightenter = self.computeEnterValues(something, enterEliminationOrder)
            expectedValue += (prob * max(rightenter,leftenter))
        "*** YOUR CODE HERE ***"
